...
.toc-content {
    @apply border-accent/10 bg-bgColor absolute right-1 bottom-0 max-h-[55vh] w-76 overflow-y-auto p-2 transition-[opacity,transform] duration-200 ease-out sm:top-0 sm:bottom-auto sm:max-h-[68vh] rounded-none border-y-2 border-x-0;
    box-shadow: none;
  }
...
.post-card-tags {
    @apply flex flex-wrap items-baseline px-0 pb-3 gap-2 uppercase tracking-[0.12em];
}
...
.post-card-badge {
  @apply bg-quote/90 absolute top-2 right-2 flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-semibold text-white rounded-none uppercase tracking-[0.14em];
}
.post-card-meta-date {
  @apply text-accent/80 mb-1 block font-mono text-[10px] uppercase tracking-[0.16em];
}
.post-card-title {
  @apply mb-1 text-sm leading-snug font-semibold font-accent tracking-[0.005em];
}
.post-card-author-link {
  @apply text-textColor/60 hover:text-accent text-[10px] transition-colors uppercase tracking-[0.08em];
}
...
/* Drop cap — only when a post opens directly with prose (skips TOC-toggle posts). */
.post-body > .notion-text:first-child p:first-child::first-letter {
  float: left;
  font-family: var(--font-accent, ui-serif, Georgia, serif);
  font-weight: 600;
  font-size: 3.4em;
  line-height: 0.66;
